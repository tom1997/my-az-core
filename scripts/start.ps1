...
if (-not (Test-Path -LiteralPath (Join-Path $bin 'worldserver.exe'))) { throw "当前 release 不完整：$releasePath" }
...
$auth = Start-Process -FilePath (Join-Path $bin 'authserver.exe') -ArgumentList $authArgs -WorkingDirectory $bin -PassThru
[IO.File]::WriteAllText((Join-Path $paths.State 'authserver.pid'), [string]$auth.Id)
$world = Start-Process -FilePath (Join-Path $bin 'worldserver.exe') -ArgumentList $worldArgs -WorkingDirectory $bin -PassThru
